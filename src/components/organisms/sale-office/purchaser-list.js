...
for (let i = 0; i < 45; i += 1) {
  purchaserData.push({ id: i, userStatus: userStatusList[Math.floor(Math.random() * 2)] });
}
  ...
  return (
    <>
      ...
      <Row mb={15} align='middle' justify='space-between'>
        <Paragraph type='secondary' mb={0} fz={24} fw={600}></Paragraph>
        ...
        pagination={{
          ...
          showTotal: (total, current) => (
            <Paragraph type='secondary' fw={400} fz={16}>
              Showing {current[0]}-{current[1]} of {purchaserData.length}
            </Paragraph>
          ),
        }}
        dataSource={purchaserData}
        renderItem={item => (
          <List.Item
            key={item.id}
            actions={[
            ...
            <List.Item.Meta
              title={
                <Row>
                  ...
                  <Col span={5} align='center'>
                    <Paragraph mb={0}>Projects (3)</Paragraph>
                  </Col>
                  <Col span={5} align='center'>
                    <Paragraph mb={0}>Jakob Vetoes</Paragraph>
                  </Col>
